...
const designTokensLight = JSON.parse(
  fs.readFileSync('./tokens/design-tokens-light.tokens.json', 'utf-8')
);
const designTokensDark = JSON.parse(
  fs.readFileSync('./tokens/design-tokens-dark.tokens.json', 'utf-8')
);
...
describe('Design Tokens Schema', () => {
  it('should have matching token structure between light and dark themes', () => {
    const lightKeys = extractTokenKeys(designTokensLight.light);
    const darkKeys = extractTokenKeys(designTokensDark.dark);
  ...
  it('should have matching button tokens between player 1 and player 2 in both themes', () => {
    const themes = [designTokensLight.light, designTokensDark.dark];
    const categories = ['button', 'text', 'surface'];

    for (const theme of themes) {
      for (const category of categories) {
        const player1Keys = extractTokenKeys(theme[category]['player 1']);
        const player2Keys = extractTokenKeys(theme[category]['player 2']);

        // Check that every player 1 category token exists in player 2
        const missingInPlayer2 = Array.from(player1Keys).filter(key => !player2Keys.has(key));
        expect(
          missingInPlayer2,
          `Missing in ${theme} ${category}.player2: ${missingInPlayer2.join(', ')}`
        ).to.be.empty;

        // Check that every player 2 category token exists in player 1
        const missingInPlayer1 = Array.from(player2Keys).filter(key => !player1Keys.has(key));
        expect(
          missingInPlayer1,
          `Missing in ${theme} ${category}.player1: ${missingInPlayer1.join(', ')}`
        ).to.be.empty;

        // Check they have the same number of tokens
        expect(player1Keys.size).to.equal(
          player2Keys.size,
          `${theme} ${category}.player1 and ${category}.player2 should have the same number of tokens`
        );
      }
    }
  });
